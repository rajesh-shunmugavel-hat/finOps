...
"""
    ...
    def get_recommendation_from_text(self, prompt: str) -> str:
        """
        A generic method to get a text-based recommendation from Gemini
        by providing a full, pre-formatted prompt.
        """
        try:
            response = self.client.generate_content(prompt)
            return response.text
        except Exception as e:
            logger.error(f"Error getting recommendation from text: {e}")
